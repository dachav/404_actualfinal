...
def get_df():
	...
	for cuisine in cuisines:
	        sql = "select avg(w.sentiment) AS 'Average Sentiment', avg(w.rating) AS 'Average Rating', count(distinct w.rid) as 'Num Rest' from reviews w, rest r WHERE w.rid = r.rid AND categories LIKE '%" + cuisine + "%';"
	        if i > 1:
...
#print df.sort_values(['Average Sentiment'], ascending = False)     
